...
export default function Router() {
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold">{t("router.title")}</h1>
      <p className="muted text-sm">guard → intent (linear probe, {RE.accuracy != null ? `${fmtPct(RE.accuracy)} on ${RE.n_test} held-out real requests` : "not yet trained"}) → cheapest adequate tier → escalate on low confidence. {live ? t("router.intro.tail.live") : t("router.intro.tail.replay")}</p>
      <Note status={live ? "measured" : RP.status} note={RP.note} />
      <Section title={t("router.section.send")}>
        ...
        <div className="flex gap-2 mt-2 items-center flex-wrap">
          <select value={force} onChange={(e) => setForce(e.target.value)}><option value="">{t("router.policyDecides")}</option>{TIER_ORDER.filter((ti) => ti !== "R5_PII").map((ti) => <option key={ti} value={ti}>force {TIER_LABEL[ti]}</option>)}</select>
          <button className="btn btn-primary" onClick={send} disabled={busy || !isConfigured()}>{busy ? t("router.routing") : t("router.route.btn")}</button>
          {!isConfigured() && <span className="muted text-xs">offline: set NEXT_PUBLIC_API_BASE</span>}
        </div>
        {last && last.hops && (
          <div className="mt-3 grid md:grid-cols-3 gap-3">
            <div className="card md:col-span-1">
              <div className="muted text-xs uppercase">{t("router.trace")}</div>
            ...
            <div className="card md:col-span-2"><div className="muted text-xs uppercase">{t("router.answer")}</div><div className="text-sm mt-1 whitespace-pre-wrap">{last.answer}</div>{last.redacted_input && last.redacted_input !== text && <div className="muted text-xs mt-2">sent upstream as: <span className="mono">{last.redacted_input}</span></div>}</div>
          </div>
        )}
      </Section>
      <div className="grid md:grid-cols-4 gap-3 mt-4">
        <StatTile label={t("router.stat.requests")} value={fmtNum(rows.length)} sub={`${rows.filter((r) => r.escalated).length} ${t("router.stat.escalated")}`} />
        <StatTile label={t("router.stat.policySpend")} value={fmtUsd(wi.policy, 4)} color="var(--series-1)" />
        <StatTile label={t("router.stat.allFlagship")} value={fmtUsd(wi.flagship, 4)} sub={`${wi.policy > 0 ? (wi.flagship / wi.policy).toFixed(1) : "—"}×`} color="var(--series-2)" />
        <StatTile label={t("router.stat.piiToFrontier")} value="0" sub={`${fmtNum(rows.reduce((a, r) => a + (r.pii?.redacted_count || 0), 0))} ${t("router.stat.redactedByGuard")}`} color="var(--series-6)" />
      </div>
      <Section title={t("router.section.whatif")} right={<span className="mono text-sm">{pct}% → {fmtUsd(wi.blended, 4)}</span>}>
        <input type="range" min={0} max={100} value={pct} onChange={(e) => setPct(+e.target.value)} className="w-full" />
        <div className="muted text-xs">{t("router.whatif.caption")} {fmtUsd(wi.small, 6)}.</div>
      </Section>
      <div className="grid md:grid-cols-2 gap-4">
        <Section title={t("router.section.costByTier")}>
        ...
        <Section title={t("router.section.cumPnl")}>
          <div style={{ height: 260 }}><ResponsiveContainer><AreaChart data={cum} margin={{ left: 10, right: 10 }}>
            ...
            <Area type="monotone" dataKey="flagship" name={t("router.legend.allFlagship")} stroke="var(--series-2)" fill="var(--series-2)" fillOpacity={0.12} strokeWidth={2} />
            <Area type="monotone" dataKey="policy" name={t("router.legend.policy")} stroke="var(--series-1)" fill="var(--series-1)" fillOpacity={0.2} strokeWidth={2} />
            <Area type="monotone" dataKey="small" name={t("router.legend.allSmall")} stroke="var(--series-3)" fill="var(--series-3)" fillOpacity={0.1} strokeWidth={2} />
          </AreaChart></ResponsiveContainer></div>
        </Section>
      </div>
      <Section title={t("router.section.lastReq")}>
        <div className="overflow-x-auto"><table className="data"><thead><tr><th>{t("router.th.request")}</th><th>{t("router.th.intent")}</th><th>{t("router.th.path")}</th><th>$</th><th>ms</th><th>PII</th></tr></thead>
      ...
      {RE.reliability?.length > 0 && <Section title={t("router.section.calibration")}><div className="flex flex-wrap gap-1">{RE.reliability.map((b) => <span key={b.bin} className="chip">conf {b.bin}: <span className="mono">{fmtPct(b.accuracy)}</span> (n={b.n})</span>)}</div></Section>}
    </div>
  );
}
