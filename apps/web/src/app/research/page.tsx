  ...
  return (
    <article style={{ maxWidth: 860, margin: "0 auto", ...serif }}>
      ...
      <header style={{ borderBottom: "2px solid var(--border)", paddingBottom: "1rem" }}>
        ...
        <p style={{ ...sysFont, fontSize: "0.78rem", color: "var(--text-muted)", marginTop: "0.5rem", marginBottom: 0 }}>September 2026 - reproducible build - every figure is generated from committed measurement files</p>
      </header>
      ...
      <section style={{ margin: "1.2rem 0", padding: "1rem 1.1rem", background: "var(--surface-2)", borderRadius: 10 }}>
        <h2 style={{ ...sysFont, fontSize: "0.8rem", letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--text-muted)", margin: "0 0 0.4rem" }}>Abstract</h2>
      ...
      <Fig n={2} caption="The path of a single request. The privacy guard runs before any model call, the router selects the workload class, and the cascade spends more only when the cheap tier is unsure.">
        <LifecycleFigure />
      </Fig>
      <p style={p}>
      ...
      <H n="4" id="s4">How it was built, stage by stage</H>
      <p style={p}>
        The system is produced by a single reproducible pipeline. Each stage runs on managed compute, writes its result to a shared volume, and is idempotent, so a failed step can be repaired and re-run without repeating the ones before it. The stages are as follows.
      </p>
      <Fig n={3} caption="The build pipeline. Training and evaluation happen on serverless GPUs; the results are committed as JSON and then served by the gateway and rendered on this site.">
        <StagesBlock />
      </Fig>

      <H n="5" id="s5">Datasets</H>
      <p style={p}>
      ...
      <div style={{ overflowX: "auto", margin: "1.1rem 0" }}>
        ...
        <p style={capNote}>Table 1. Every corpus used for training and evaluation, with its source and the transformations applied.</p>
      </div>
      <p style={p}>
      ...
      <div style={{ overflowX: "auto", margin: "1.1rem 0" }}>
        ...
        <p style={capNote}>Table 2. The router corpus, {fmtInt(R.n_train + R.n_test)} examples across {R.labels.length} classes, assembled from the sources above and split into {fmtInt(R.n_train)} train and {fmtInt(R.n_test)} test.</p>
      </div>

      <H n="6" id="s6">Sample rows</H>
      <p style={p}>
        The records below are genuine rows loaded from the public datasets, shown so the inputs and labels can be inspected directly rather than taken on trust.
      </p>
      <div style={{ margin: "0.6rem 0 0.2rem" }}>
        <p style={{ ...sysFont, fontSize: "0.9rem", fontWeight: 700, margin: "0.8rem 0 0.3rem" }}>Transactions (narration to spending category)</p>
        <SampleTable head={["Narration", "Category"]} rows={TXN_SAMPLES} mono />
        <p style={capNote}>The leading marker in brackets is part of the source formatting and is kept as-is.</p>
      </div>
      <div style={{ margin: "0.6rem 0 0.2rem" }}>
        <p style={{ ...sysFont, fontSize: "0.9rem", fontWeight: 700, margin: "0.8rem 0 0.3rem" }}>Banking77 (customer question to intent)</p>
        <SampleTable head={["Question", "Intent (1 of 77)"]} rows={B77_SAMPLES} />
      </div>
      <div style={{ margin: "0.6rem 0 0.2rem" }}>
        <p style={{ ...sysFont, fontSize: "0.9rem", fontWeight: 700, margin: "0.8rem 0 0.3rem" }}>Personal data (before to after redaction)</p>
        <SampleTable head={["Original text", "After the guardrail"]} rows={PII_SAMPLES} mono />
        <p style={capNote}>The first pair is a training example from ai4privacy; the second is the live output of the deployed guardrail on the card-and-email test string.</p>
      </div>
      <div style={{ margin: "0.6rem 0 0.2rem" }}>
        <p style={{ ...sysFont, fontSize: "0.9rem", fontWeight: 700, margin: "0.8rem 0 0.3rem" }}>Router corpus (a genuine row from three of the source datasets)</p>
        <SampleTable head={["Class", "Source", "Example row"]} rows={ROUTER_SAMPLES} mono />
      </div>

      <H n="7" id="s7">Results</H>
      <p style={p}>
      ...
      <Fig n={4} caption="Accuracy against cost per one million classifications. Filled points are the standard test split; hollow points are the unseen-merchant split. The dashed drop for the owned model is the whole argument for the cascade: it is unbeaten on familiar inputs and weakest on novel ones.">
        <ScatterFigure />
      </Fig>
      <p style={p}>
      ...
      <Fig n={5} caption="Entity-level detection of personal data on held-out documents. A learned model plus regular expressions recovers the entities that pattern matching cannot see, notably names and addresses.">
        <PiiFigure />
      </Fig>
      ...
      <Fig n={6} caption="Illustrative monthly cost by volume. Self-hosting wins only above the crossover, and the crossover slides right as frontier prices fall. The interactive version, with configurable numbers, is on the Underwrite tab.">
        <UnderwriteFigure />
      </Fig>
      <p style={p}>
      ...
      <H n="9" id="s9">What this demonstrates</H>
      <p style={p}>
      ...
      <H n="11" id="s11">Reproduce it live</H>
      <p style={p}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "0.6rem", ...sysFont, margin: "1rem 0 0.4rem" }}>
        {[
          ...
          ["/guardrail", "Guardrail", "redact personal data in real text"],
          ["/batch", "Batch", "ten thousand transactions versus frontier"],
          ["/underwrite", "Underwrite", "the crossover with configurable numbers"],
        ].map(([href, title, sub]) => (
